...
function getFileName(containerName: string, path: string, file: StrapiFile) {
  ...
  if (isRootContainer(containerName) || trimParam(path) === "") return name;
...
async function handleUpload(
  ...
  const client = containerClient.getBlockBlobClient(
    getFileName(config.containerName, config.defaultPath, file),
  );
  ...
  file.url = cdnBaseURL
    ? cleanUrl.replace(serviceBaseURL, cdnBaseURL)
    : cleanUrl;
    ...
    return {
      ...
      async getSignedUrl(file: StrapiFile) {
        ...
        const expiresOn = new Date(
          now.getTime() + config.sasMinutes * 60 * 1000,
        );
        ...
        try {
          ...
          if (config.authType === "msi") {
            const userDelegationKey = await blobSvcClient.getUserDelegationKey(
              now,
              expiresOn,
            );
      ...
      async getStream(
        ...
        try {
          // Azure SDK download accepts (offset, count)
          ...
          return {
            ...
            contentLength: downloadResponse.contentLength, // This will be the size of the CHUNK
            contentRange: downloadResponse.contentRange, // e.g., "bytes 0-500/1000"
            acceptRanges: "bytes",
          };
        } catch (err) {
          throw new Error("Error fetching stream from Azure");
        }
      },
      isPrivate() {
